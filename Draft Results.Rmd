...
favstats(~EDUCLEVEL, data=usa_data4)
tally(~METRO, data=usa_data4)
tally(~RACE4, data=usa_data4)
...
```{r}
m1 <- lm(INCWAGE~EDUCLEVEL+METRO+SEX+AGE+SCHLTYPE, data=usa_data_clean)
summary(m1)
```
...
```{r}
m2 <- lm(INCWAGE~EDUCLEVEL+RACE4+EDUCLEVEL*RACE4, data=usa_data_clean)
summary(m2)
```
